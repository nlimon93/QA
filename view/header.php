...
<head>
	...
	<link rel="stylesheet" href="/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/resources/Stylesheets/main.css" type="text/css">
	<link rel="stylesheet" href="/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/resources/Stylesheets/login.css" type="text/css">
	<link rel="stylesheet" href="local.css" type="text/css">
	<script src="/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/resources/javascript/main.js" type="text/javascript" async="true"></script>
...
<header>
	...
	<div id="login">
		...
		<?php elseif (array_key_exists('loggedin', $_SESSION) == true) : ?>
			...
			<button id="accountbtn" class="raised" onclick="window.location.href = '/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/Account'"><?php echo $_SESSION['name']?></button>

			<form action="/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/Login/Logout.php" method="POST">
    ...
    <div id="logform" class="modal">
		...
		<form class="modal-content Mform animate" action="/<?php $ex = explode(DIRECTORY_SEPARATOR ,__DIR__); $rev = array_reverse($ex); echo $rev[1] ?>/Login/loginAttempt.php" method="POST">
			<div class="container">
